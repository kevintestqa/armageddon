...
resource "aws_wafv2_web_acl_logging_configuration" "satellite_waf_logging01" {
  ...
  log_destination_configs = [
    aws_cloudwatch_log_group.satellite_waf_log_group01[0].arn
  ]
...
resource "aws_wafv2_web_acl_logging_configuration" "satellite_waf_logging_s3_01" {
  ...
  resource_arn = aws_wafv2_web_acl.satellite_waf01[0].arn
  log_destination_configs = [
    aws_s3_bucket.satellite_waf_logs_bucket01[0].arn
  ]
...
resource "aws_iam_role_policy" "satellite_firehose_policy01" {
  ...
  role  = aws_iam_role.satellite_firehose_role01[0].id
...
resource "aws_kinesis_firehose_delivery_stream" "satellite_waf_firehose01" {
  ...
  extended_s3_configuration {
    role_arn   = aws_iam_role.satellite_firehose_role01[0].arn
    bucket_arn = aws_s3_bucket.satellite_firehose_waf_dest_bucket01[0].arn
    prefix     = "waf-logs/"
  }
}
...
resource "aws_wafv2_web_acl_logging_configuration" "satellite_waf_logging_firehose01" {
  ...
  resource_arn = aws_wafv2_web_acl.satellite_waf01[0].arn
  log_destination_configs = [
    aws_kinesis_firehose_delivery_stream.satellite_waf_firehose01[0].arn
  ]
