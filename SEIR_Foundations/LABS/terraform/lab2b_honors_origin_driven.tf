...
# Explanation: satellite uses AWS-managed policies—battle-tested configs so students learn the real names.
data "aws_cloudfront_cache_policy" "satellite_use_origin_cache_headers01" {
  name = "UseOriginCacheControlHeaders"
}

# Explanation: Same idea, but includes query strings in the cache key when your API truly varies by them.
data "aws_cloudfront_cache_policy" "satellite_use_origin_cache_headers_qs01" {
...
data "aws_cloudfront_origin_request_policy" "satellite_orp_all_viewer01" {
  name = "Managed-AllViewer"
}

data "aws_cloudfront_origin_request_policy" "satellite_orp_all_viewer_except_host01" {
...
# Explanation: Public feed is cacheable—but only if the origin explicitly says so. satellite demands consent.
...
# Explanation: Everything else under /api is dangerous by default—satellite disables caching until proven safe.
# ordered_cache_behavior {
#   path_pattern           = "/api/*"
#   target_origin_id       = "${var.project_name}-alb-origin01"
#   viewer_protocol_policy = "redirect-to-https"
