...
# resource "aws_s3_bucket" "satellite_ir_reports_bucket01" {
#   bucket = "${var.project_name}-ir-reports-${data.aws_caller_identity.satellite_self01.account_id}"
...
# resource "aws_s3_bucket_public_access_block" "satellite_ir_reports_pab01" {
#   bucket                  = aws_s3_bucket.satellite_ir_reports_bucket01.id
...
# resource "aws_iam_role" "satellite_ir_lambda_role01" {
...
# resource "aws_iam_policy" "satellite_ir_lambda_policy01" {
...
#         ],
#         Resource = "arn:aws:ssm:*:${data.aws_caller_identity.satellite_self01.account_id}:parameter/lab/db/*"
#       },
#       # Secrets Manager
#       {
#         Effect = "Allow",
...
#         ],
#         Resource = "arn:aws:secretsmanager:*:${data.aws_caller_identity.satellite_self01.account_id}:secret:${var.project_name}/rds/mysql*"
#       },
#       # S3 report write
#       {
#         Effect = "Allow",
...
#         ],
#         Resource = [
#           aws_s3_bucket.satellite_ir_reports_bucket01.arn,
#           "${aws_s3_bucket.satellite_ir_reports_bucket01.arn}/*"
#         ]
#       },
#       # Bedrock invoke
...
# resource "aws_iam_role_policy_attachment" "satellite_ir_lambda_attach01" {
#   role       = aws_iam_role.satellite_ir_lambda_role01.name
#   policy_arn = aws_iam_policy.satellite_ir_lambda_policy01.arn
# }

# # Explanation: Basic Lambda logging—because even droids need diaries.
# resource "aws_iam_role_policy_attachment" "satellite_ir_lambda_basiclogs01" {
#   role       = aws_iam_role.satellite_ir_lambda_role01.name
...
# resource "aws_lambda_function" "satellite_ir_lambda01" {
#   function_name = "${var.project_name}-ir-reporter01"
#   role          = aws_iam_role.satellite_ir_lambda_role01.arn
...
#   environment {
#     variables = {
#       REPORT_BUCKET        = aws_s3_bucket.satellite_ir_reports_bucket01.bucket
...
#       SNS_TOPIC_ARN        = aws_sns_topic.satellite_sns_topic01.arn
#     }
#   }
# }

# # Explanation: This subscription wires the pager to the scribe—SNS triggers Lambda when alarms fire.
# resource "aws_sns_topic_subscription" "satellite_ir_lambda_sub01" {
#   topic_arn = aws_sns_topic.satellite_sns_topic01.arn
#   protocol  = "lambda"
#   endpoint  = aws_lambda_function.satellite_ir_lambda01.arn
# }

# # Explanation: Allow SNS to invoke Lambda—Chewbacca authorizes the distress beacon to wake the droid.
# resource "aws_lambda_permission" "satellite_allow_sns_invoke01" {
#   statement_id  = "AllowExecutionFromSNS"
#   action        = "lambda:InvokeFunction"
#   function_name = aws_lambda_function.satellite_ir_lambda01.function_name
#   principal     = "sns.amazonaws.com"
#   source_arn    = aws_sns_topic.satellite_sns_topic01.arn
# }

# # Explanation: Output report bucket—Chewbacca needs the archive coordinates for grading.
# output "satellite_ir_reports_bucket" {
#   value = aws_s3_bucket.satellite_ir_reports_bucket01.bucket
# }
